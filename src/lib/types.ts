...
export interface Publisher {
  ...
  // Phase 104 — profile photo/logo (schema_phase104_publisher_profile_image.sql).
  // Public URL in the profile-images bucket, 2MB cap enforced by the bucket
  // itself. Null (the common case until publishers backfill one) falls back
  // to the initials-on-swatch avatar — see PublisherAvatar.tsx, the single
  // place that fallback lives.
  profile_image_url: string | null;
  // 12-Channel Audit fix A1 (schema_phase97_verification_proof_upload.sql)
  // — storage PATHS (not public URLs; the bucket is private) in
  // publisher-verification-proof, up to 5. Only ever meaningful for
  // verification_required channels; empty for the rest.
...
export interface Profile {
  ...
  // Phase 81 (Business Value Proposition & Opportunities Engine) — a
  // controlled business-type taxonomy, kept alongside the free-text
  // `industry` field above rather than replacing it.
...
export interface PublisherRequest {
  ...
  // Only full_name/company_name — the only fields business_contact_public
  // exposes (schema_phase84_safe_cross_party_profile_access.sql; see
  // src/lib/businessContact.ts). Used to also carry phone/email_verified/
  // phone_verified/business_verified via a direct profiles embed, but
  // nothing in this codebase ever read those fields off this type, and
  // that embed was the exact column-over-exposure "Claude To fix 1..txt"
  // item 8 flagged.
...
export interface ChannelRequest {
  ...
  // Per-channel structured request fields (preferred air date, landing
  // page URL, artwork-ready, etc.) the business filled in beyond the
  // generic fields above. See src/lib/channelRequestFieldSchemas.ts for
  // the full typed shape per channel_slug.
  ...
  // Only full_name/company_name/verification_level — what
  // business_contact_public exposes (schema_phase98). Used to be a
  // direct profiles embed including phone, which silently returned null
  // for every non-admin/non-owner viewer once
  // schema_phase84_safe_cross_party_profile_access.sql removed the
  // shared-access policies profiles relied on — that embed was never
  // updated for channel_requests specifically. See
...
export interface AgencyLead {
  ...
  // An admin, or (schema_phase94_manager_internal_roles.sql) a profile
  // with the 'campaign_manager' role.
  campaign_manager_id: string | null;
  // Same as campaign_manager_id, for the 'publisher_manager' role —
  // supply-side rather than demand-side. See schema_phase94.
...
export interface AgencyCampaign {
  ...
  client_id: string;
  campaign_manager_id: string | null;
  // See the same field on AgencyLead above (schema_phase94).
  publisher_manager_id: string | null;
  name: string;
...
export interface BusinessRelationship {
  ...
  business_name: string;
  campaign_count: number;
  // Raw gross booking amount, NOT net of commission — multiply by
  // PUBLISHER_SHARE (constants.ts) to get what the publisher actually
  // earned. Renamed from total_earned (schema_phase99) — that name
  // implied net-of-commission while a stale hardcoded 0.88 multiplier
  // computed it in SQL; see that migration's comment for why duplicating
  // the commission rate in SQL is exactly what this codebase avoids
  // everywhere else (analytics_functions.sql).
  total_spent: number;
  last_campaign_at: string | null;
}
...
// ChatSched Tools catalogue — see schema_phase100_chatsched_tools.sql.
